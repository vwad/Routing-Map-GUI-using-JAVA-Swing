...
public class SingleClosureThread implements Runnable{
    ...
    protected SingleClosureThread(Connection c, Location city, List<ClosureInfo> info, int start, int end) throws SQLException {
        Statement statement = c.createStatement();
        statement.executeUpdate("DROP TABLE IF EXISTS RemovedStops");
        statement.executeUpdate("CREATE TABLE RemovedStops (removedStop text not null , lostTime double not null , tripID integer not null , Foreign key (tripID) references Closure(tripID), foreign key (removedStop) references Stops (stop_id))");

        this.city = city;
        this.prepStatement = c.prepareStatement("INSERT INTO RemovedStops VALUES (?,?,?)");;
