...
public class TripleClosureThread implements Runnable{
    ...
    public TripleClosureThread(Connection c, Location city, List<ClosureInfo> info, int start, int end) throws SQLException {
        Statement statement = c.createStatement();
        statement.executeUpdate("DROP TABLE IF EXISTS RemovedThreeStops");
        statement.executeUpdate("CREATE TABLE RemovedThreeStops ( " +
                "removedStop1 text not null , " +
                "removedStop2 text not null , " +
                "removedStop3 text not null , " +
                "lostTime double not null , " +
                "tripID integer not null , " +
                "Foreign key (tripID) references Closure(tripID), " +
        ...
        this.city = city;
        this.prepStatement = c.prepareStatement("INSERT INTO RemovedThreeStops VALUES (?,?,?,?,?)");;
    ...
    public void getStopClosures() throws SQLException {
        ...
        Statement statement = c.createStatement();
        ResultSet resultSet = statement.executeQuery("select avg(lostTime), removedStop " +
                "from RemovedStops " +
                "where lostTime is not null " +
                "group by removedStop " +
                "order by avg(lostTime) desc " +
                "limit 20;");
        ...
        TripleClosureThread.combinations = combinations;
    }
